...
const AdminAwaitingReviews = () => {
  ...
  // Fetch awaiting review complaints
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get('/admin/awaiting-review', {
  ...
  const handleApprove = async (id) => {
    try {
      await axios.patch(`/admin/complete/${id}`, {}, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      });
      toast.success('Complaint approved and marked as completed');
    ...
    <div className="min-h-screen bg-gray-50 p-4 md:p-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-gray-800">Awaiting Reviews (Admin)</h1>
            <p className="text-gray-600">
              {filteredComplaints.length} {filteredComplaints.length === 1 ? 'complaint' : 'complaints'} awaiting your review
            </p>
          </div>
        ...
        {!loading && filteredComplaints.length === 0 && (
          <div className="bg-white rounded-xl shadow p-8 text-center">
            ...
            <h2 className="text-xl font-semibold mb-2">No complaints awaiting review</h2>
            <p className="text-gray-500">All complaints have been reviewed!</p>
          </div>
        )}
            ...
            <div key={complaint._id} className="bg-white rounded-xl shadow-md overflow-hidden">
              <div className="p-6">
                ...
                {complaint.assignedTo && (
                  <div className="mt-2">
                    <p className="text-sm text-gray-700">
                      <span className="font-medium">Assigned Staff:</span> {complaint.assignedTo.name || 'N/A'}
                    </p>
                    <p className="text-sm text-gray-700">
...
export default AdminAwaitingReviews;
