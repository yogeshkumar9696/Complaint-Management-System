...
const StaffAwaitingReview = () => {
  ...
  useEffect(() => {
    const fetchAwaitingReviewComplaints = async () => {
      try {
        const response = await axios.get('/staff/awaiting-review', {
          headers: {
        ...
        toast.error(err.response?.data?.error || 'Failed to load awaiting review tasks');
      } finally {
        setLoading(false);
      }
    };

    fetchAwaitingReviewComplaints();
  }, []);
    ...
    <div className="min-h-screen bg-gray-50 p-4 md:p-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-gray-800">Awaiting Review</h1>
            <p className="text-gray-600">
              {filteredComplaints.length} {filteredComplaints.length === 1 ? 'task' : 'tasks'} awaiting admin review
            </p>
          </div>
          ...
          <div className="bg-white rounded-xl shadow p-8 text-center">
            ...
            <p className="text-gray-500">
              {searchTerm || categoryFilter !== 'all' || dateFilter !== 'all'
                ...
                : 'Tasks you resolved will appear here while awaiting admin approval'}
            </p>
          </div>
        )}
            ...
            <div key={complaint._id} className="bg-white rounded-xl shadow-md overflow-hidden">
              <div className="p-6">
                ...
                <div className="flex flex-wrap gap-3 mt-2">
                  ...
                  <span className="px-3 py-1 rounded-full bg-gray-100 text-gray-800 text-sm font-medium">
                    Submitted: {new Date(complaint.updatedAt).toLocaleDateString()}
                  </span>
                </div>
                ...
                {complaint.attachments.url && (
                  <div className="mt-3">
                    <p className="text-sm font-medium text-gray-700 mb-1">Original Complaint Image:</p>
                    <img
                ...
                {complaint.resolutionProof && (
                  <div className="mt-3">
                    <p className="text-sm font-medium text-gray-700 mb-1">Your Resolution Proof:</p>
                    <img
...
export default StaffAwaitingReview;
